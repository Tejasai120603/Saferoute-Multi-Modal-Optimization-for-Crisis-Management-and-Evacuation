...
earthquake_model = joblib.load("C:/Users/suman/Downloads/project (2) (1)/project (2)/project/Earthquake_Model.pkl")

flood_model = joblib.load("project (2)/project/Flood_Model.pkl")
flood_data_path = "project (2)/project/encoded_flood_data_with_waterlevel.csv"
...
def display_precautions(magnitude, population_density, building_age):
    ...
    if magnitude < 4:
        ...
        precautions.append("Minor impact. Remain alert.Carry resources like medicine if any complications.")
    elif 5 <= magnitude < 6:
        precautions.append(
            "Moderate impact. Evacuate high-rise buildings in crowded areas.Carry resources like head gear etc.,"
        )
    elif 6 <= magnitude < 7:
        precautions.append(
            "Significant impact. Evacuate densely populated high-rise buildings.Carry any protective gear for safety."
        )
    elif 7 <= magnitude < 8:
        precautions.append(
            "Severe impact. Immediate evacuation of high-rise buildings.Carry the any protective gear."
        )
    else:
        precautions.append("Devastating impact. Large-scale evacuation needed.Carry necessary food items.")
    if population_density == "high":
        precautions.append("Prepare for potential mass evacuations.Carry high calorie food items.Carry any protective gear for safety.")
    if building_age == "old (20-50 years)":
        precautions.append("Inspect older buildings for structural vulnerabilities.Carry any protective gear for safety.")
    return "\n".join(precautions)
...
async def predict(request: Request):
    ...
    if input_data["prediction_type"] == "earthquake":
        ...
        if risk_score >= 8:
            ...
            precautions = "Evacuate if necessary. Move to higher ground.Carry any survival kit if available."
        elif risk_score >= 5:
            alert_level = "High Risk - Remain on alert."
            precautions = "Prepare for potential evacuation.Carry the necessary food items."
        else:
            alert_level = "Moderate Risk - Minimal action required."
            precautions = "Monitor updates regularly.Carry food if necessary.And medical first aid kit if necessary."
